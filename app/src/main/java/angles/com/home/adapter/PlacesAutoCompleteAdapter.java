package angles.com.home.adapter;
...
/*
		...
		Log.d("Autocomplete api","*** "+apiService.toString());

        call = apiService.getCityResults("", input, "", radius, context.getString(R.string.map_api_key_place));
        call.enqueue(new Callback<PlaceResults>() {
            @Override
            public void onResponse(Call<PlaceResults> call, Response<PlaceResults> response) {
            //    Log.d(LOG_TAG, "onSuccess body: " + response);
                try {
					Log.d(LOG_TAG, "****response " + response.toString());

					if (response.isSuccessful()) {
                        Log.d(LOG_TAG, "****onSuccess body: " + response.body().toString());
                        //Gson gson=new Gson();
                        String rsponseBody = response.body().toString();
                        if (rsponseBody != null) {
							Log.d("Autocomplete ","*** getPredictions()"+response.body().getPredictions());

                            PlaceResults places = response.body();
                            List<Predictions> predsJsonArray=response.body().getPredictions();
                            resultList.clear();
                            for (int i = 0; i < predsJsonArray.size(); i++) {
                                resultList.add(predsJsonArray.get(i).getDescription());
                            }
                            //Dataget();
                        }
                        else {
